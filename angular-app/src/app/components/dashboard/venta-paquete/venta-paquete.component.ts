...
export class VentaPaqueteComponent implements OnInit {
  ...
  doPurchase() {
  const {clientName, idpackage, quantityAdults, quantityKids} = this.form.value;
  if(Number(idpackage)){
    if(Number(quantityAdults)&&Number(quantityKids)){
      if(quantityAdults+quantityKids<=10){

        this.userService.doSell(clientName,+idpackage, quantityAdults, quantityKids).subscribe(resp=>{
