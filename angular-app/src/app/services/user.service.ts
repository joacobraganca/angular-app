...
export class UserService {
  ...
  getPackages():Observable<PackagesResponse> {
    ...
    return this.http.get<PackagesResponse>('https://destinos.develotion.com/paquetes.php', {
      headers
    });
  }

  doSell(nombreCliente: string, idPaquete:number, cantidadMayores:number, cantidadMenores:number) {
    const headers = { 'Content-type': 'application/json', 'apikey':this.getApiKey() };
  ...
  getSells():Observable<SellsResponse> {
    const headers = { 'Content-type': 'application/json', 'apikey':this.getApiKey() };
    return this.http.get<SellsResponse>('https://destinos.develotion.com/ventas.php', {
      headers,
      params:{
